...
for i in range(width_max-2):
    ...
    for j in range(height_max):
        ...
        if(angle<y):
            data[j1,i] = [int(255*n_norm), int(255*n_norm*math.sin(angle/y)), 0]
        elif(angle<g):
            data[j1,i] = [int(255*n_norm*math.sin(1-(angle-y)/(g-y))), int(255*n_norm), 0]
        elif(angle<b):
            data[j1,i] = [0, int(255*n_norm*math.sin(1-(angle-g)/(b-g))), int(255*n_norm*math.sin((angle-g)/(b-g)))]
        elif(angle<360):
            data[j1,i] = [int(255*n_norm*math.sin((angle-b)/(360-b))),0, int(255*n_norm*math.sin(1-(angle-b)/(360-b)))]
    if(mod==0):
        mod=1
    else:
        mod=0
